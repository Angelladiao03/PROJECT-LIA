...
async function loadSubmittedReports() {
  ...
  try {
    ...
    tableBody.innerHTML =
      '<tr class="empty-report-row"><td colspan="6">Could not load reports. Please try again.</td></tr>';
    return;
  }
  ...
  if (!studentReports.length) {
    tableBody.innerHTML =
      '<tr class="empty-report-row"><td colspan="6">No reports submitted yet.</td></tr>';
    return;
  }
...
function openEditProfileModal() {
  ...
  document.getElementById("editUsername").value = document
    .getElementById("displayUsername")
    .textContent.replace("@", "");
  document.getElementById("editLrn").value =
    document.getElementById("displayLrn").textContent;
  document.getElementById("editGradeSection").value = document.getElementById(
    "displayGradeSection",
  ).textContent;
  document.getElementById("editAdviser").value =
    document.getElementById("displayAdviser").textContent;
  document.getElementById("editGmail").value =
    document.getElementById("displayGmail").textContent;
...
function handleProfileUpdate(e) {
  ...
  const newGradeSection = document
    .getElementById("editGradeSection")
    .value.trim();
...
async function saveProfileUpdate(newUsername, newGradeSection, newAdviser) {
  ...
  const activeUser = JSON.parse(
    localStorage.getItem("lagroInActionActiveUser"),
  );
...
function selectReportStatus(status) {
  const tab = [...document.querySelectorAll(".tab-btn")].find((button) =>
