...
async function loadDashboardData() {
  ...
  const counts = {
    ...
    pendingReportsCount: reports.filter((report) => report.status === "Pending")
      .length,
    investigationReportsCount: reports.filter(
      (report) =>
        report.status === "Active" || report.status === "Under Investigation",
    ).length,
    resolvedReportsCount: reports.filter(
  ...
  if (!alerts.length) {
    sosBody.innerHTML =
      '<tr class="empty-record-row"><td colspan="5">No emergency SOS cases found.</td></tr>';
  }
...
function renderCharts(reports) {
  ...
  new Chart(document.getElementById("categoryChart"), {
    type: "doughnut",
    data: {
      labels: categories.length ? categories : ["No reports"],
      datasets: [
        {
