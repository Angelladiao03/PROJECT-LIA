...
async function approveReport(btn) {
    ...
    if (!ok) {
        showPagePopup('Could not approve this report. Please try again.', 'Approval Failed');
        return;
    }
...
async function rejectReport(btn) {
    ...
    if (!ok) {
        showPagePopup('Could not reject this report. Please try again.', 'Rejection Failed');
        return;
    }
    row.style.transition = 'all 0.3s ease';
    row.style.opacity = '0';
...
async function updateStatus(btn, targetStatus) {
    ...
    if (targetStatus === 'Investigation') {
        ...
        if (!ok) {
            showPagePopup('Could not update this case. Please try again.', 'Update Failed');
            return;
        }
        ...
        const ok = await postReportAction({ action: 'resolve', reportNo, isAnonymous: String(isAnonymous) });
        if (!ok) {
            showPagePopup('Could not resolve this case. Please try again.', 'Update Failed');
            return;
        }
        row.style.transition = 'all 0.3s ease';
        row.style.opacity = '0';
...
async function dispatchSOS(btn) {
    ...
    if (!ok) {
        showPagePopup('Could not dispatch a guard. Please try again.', 'Dispatch Failed');
        return;
    }
    loadSavedReportsAndAlerts();
}
...
async function respondSOS(btn) {
    ...
    if (!ok) {
        showPagePopup('Could not update this alert. Please try again.', 'Update Failed');
        return;
    }
    loadSavedReportsAndAlerts();
}
...
function openStoredReport(reportNo) {
    const report = cachedReports.find(item => item.reportNo === reportNo);
    if (!report) return;
    document.getElementById('modalCaseNo').textContent = `REPORT CASE NO. #${report.reportNo}`;
    const isAnon = report.isAnonymous === 'true';
    document.getElementById('accountInfoSection').style.display = isAnon ? 'none' : 'block';
    document.getElementById('modalDivider').style.display = isAnon ? 'none' : 'block';
