...
function viewStoredRecord(reportNo) {
    const report = cachedResolvedReports.find(item => item.reportNo === reportNo);
    if (!report) return;
    viewRecord({ caseId: `#${report.reportNo}`, fullName: report.fullName, gradeSection: report.gradeSection || '-', username: report.username, adviser: report.adviser || '-', lrn: report.lrn, email: report.email || '-', category: report.category, dateTime: report.dateTime, location: report.location, description: report.description });
}
...
function deleteRecord(rowId, caseId) {
    ...
    const report = cachedResolvedReports.find(item => item.reportNo === reportNo);
    const anonymousNotice = report?.isAnonymous === 'true'
        ? ' This anonymous report will be permanently deleted now that the case is resolved, so your data will not be stored.'
        : '';
    showPageConfirmation(`Are you sure you want to delete record ${caseId}? This action cannot be undone.${anonymousNotice}`, 'Confirm Report Deletion', async () => {
        const res = await fetch('../../../AdminReportServlet', {
            method: 'POST',
        ...
        if (!data.success) {
            showPagePopup('Could not delete this record. Please try again.', 'Delete Failed');
            return;
        }
