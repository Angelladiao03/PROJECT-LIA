...
// kick back to login once the session's gone
...
async function loadPendingRequests() {
  ...
  try {
    ...
    if (!registrations.length) {
      tableBody.innerHTML =
        '<tr class="empty-record-row"><td colspan="4">No pending registration requests.</td></tr>';
    }
    ...
    tableBody.innerHTML =
      '<tr class="empty-record-row"><td colspan="4">Could not load registration requests.</td></tr>';
  }
}
...
async function approveStudent(id, name) {
  ...
  if (!ok) {
    showPagePopup(
      "Could not approve this student. Please try again.",
      "Approval Failed",
    );
    return;
  }
  ...
  if (row) {
    ...
    setTimeout(() => {
      ...
      updatePendingCount();
      showPagePopup(
        `Account request for ${name} has been approved.`,
        "Account Approved",
      );
    }, 300);
  }
