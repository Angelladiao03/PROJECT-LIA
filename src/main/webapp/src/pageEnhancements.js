(function initializeLiaPage() {
  ...
  loader.innerHTML =
    '<div class="loader-content"><div class="loader-spinner"></div><span>Loading...</span></div>';
  document.body.prepend(loader);

  window.showPagePopup = function showPagePopup(
    message,
    title = "LagroInAction",
    onOkay,
  ) {
    ...
    if (!popup) {
      ...
      popup.innerHTML =
        '<div class="lia-popup-card" role="dialog" aria-modal="true"><h2></h2><p></p><button type="button">Okay</button></div>';
  ...
  window.showPageConfirmation = function showPageConfirmation(
    message,
    title,
    onConfirm,
    confirmLabel = "Delete",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      activeUser = JSON.parse(
        localStorage.getItem("lagroInActionActiveUser") || "null",
      );
    } catch (error) {
      activeUser = null;
    }
    ...
    document.querySelectorAll("a[href]").forEach((link) => {
      ...
      if (
        ...
        return;
      if (
        window.location.pathname.includes("/studentReports/") &&
        href.includes("studentReports.html")
      )
        return;
      link.addEventListener("click", () => window.showPageLoading());
    });
    document
      .querySelectorAll('button[onclick*="window.location"]')
      .forEach((button) => {
        button.addEventListener("click", () => window.showPageLoading());
      });
    document.querySelectorAll(".menu-btn").forEach((button) => {
      const sidebar =
        button.closest("header")?.parentElement?.querySelector(".sidebar") ||
    ...
    document.querySelectorAll(".sidebar").forEach((sidebar) => {
      ...
      const messageItem = links
        .find((link) => link.textContent.trim() === "Message")
        ?.closest(".nav-item");
      const accountItem = links
        .find((link) => link.textContent.trim() === "My Account")
        ?.closest(".nav-item");
      if (messageItem && accountItem)
        accountItem.parentElement.insertBefore(messageItem, accountItem);

      const isAdminPage = window.location.pathname.includes("/Admin/");
      if (
        (activeUser?.role === "admin" || isAdminPage) &&
        !sidebar.querySelector(".sidebar-bottom")
      ) {
        sidebar.insertAdjacentHTML(
          "beforeend",
          '<div class="sidebar-bottom"><button type="button" class="sidebar-logout" onclick="handlePageLogout()">Log Out</button></div>',
        );
      }
      ...
      if (
        ...
        isAccountPage &&
        accountItem &&
        !accountItem.querySelector(".account-menu")
      ) {
        accountItem.insertAdjacentHTML(
          "beforeend",
          '<ul class="account-menu open"><li><button type="button" class="sidebar-logout" onclick="handlePageLogout()">Log Out</button></li></ul>',
        );
